...
func (e *TestDataExecutor) Execute(ctx context.Context, queries tsdb.QuerySlice, timerange *tsdb.TimeRange) *tsdb.BatchResult {
	...
	for _, query := range queries {
		...
		if scenario, exist := ScenarioRegistry[scenarioId]; exist {
			result.QueryResults[query.RefId] = scenario.Handler(query, timerange)
